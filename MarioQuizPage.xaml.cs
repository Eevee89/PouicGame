...
public partial class MarioQuizPage : ContentPage
    // Page de quiz Mario
{
    ...
    public MarioQuizPage(GlobalDataList globalDL)
    {
        InitializeComponent();
        rootpath = AppContext.BaseDirectory.Split("PouicGame\\bin")[0];
        this.globalDL = globalDL;
        CinqBtn.IsEnabled = globalDL.mariodatas.enabledbtn[0];
        AppelBtn.IsEnabled = globalDL.mariodatas.enabledbtn[1];
        PublicBtn.IsEnabled = globalDL.mariodatas.enabledbtn[2];
        Btn();
        ManageArrows();
        string[] lines;
        if (globalDL.mariodatas.niveau == "Aspirant") { lines = File.ReadAllLines(rootpath + "PouicGameDatas\\quiz_mario_fac.txt"); }
        else { lines = File.ReadAllLines(rootpath + "PouicGameDatas\\quiz_mario_dif.txt"); }
        ...
        PriceScale.ItemsSource = globalDL.mariodatas.prix;
        if (globalDL.mariodatas.price < questions.Length)
        {
            int i = globalDL.mariodatas.price;
    ...
    private void ManageArrows()
        // Gere l'activation/désactivation des flèches du mur des prix
    {
        ...
        if (globalDL.mariodatas.price == 1) { Cursor15Btn.IsEnabled = true; }
        else if (globalDL.mariodatas.price > 1)
        {
            arrows[globalDL.mariodatas.price - 1].IsEnabled = true;
            arrows[globalDL.mariodatas.price - 2].IsEnabled = false;
        }
    }
    ...
    private async void TryAns(object sender, EventArgs e)
    {
        ...
        if (globalDL.mariodatas.price == 15)
        {
            await Task.Delay(1000);
            await Navigation.PushModalAsync(new MarioLastPage(globalDL));
        }
        else
        {
    ...
    private void JokerCinquante(object sender, EventArgs e)
    {
        ...
        globalDL.mariodatas.enabledbtn[0] = false;
    }
    ...
    private async void JokerAppel(object sender, EventArgs e)
    {
        ...
        globalDL.mariodatas.enabledbtn[1] = false;
    }
    ...
    private async void JokerPublic(object sender, EventArgs e)
    {
        ...
        globalDL.mariodatas.enabledbtn[2] = false;
    }
}